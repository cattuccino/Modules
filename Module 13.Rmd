...
Form the entries in the ANOVA table for regression
...
abline(v = fcrit) #take this section out to see the major differences!
abline(h = 0)
...
standard error of each predicted value of y:
```{r}
...
plot(m)
```
*have to expand rstudio to fit screen to see properly
...
I love this last one but it makes little sense (pls explain in class)
...
The following chart and graphs shows some other common numerical transformations that are often useful for changing a variable’s distribution to more closely approximate the normal. (reference module)
